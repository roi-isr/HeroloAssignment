...
class User(Base):
    ...
    @classmethod
    def get_user_by_name(cls, db_session, username: str, create_if_not_exist=True):
    ...
    @classmethod
    def find_user_by_id(cls, db_session, _id: int):
        user = db_session.query(cls).filter_by(id=_id).first()
    ...
    def get_all_messages(cls, db_session, username: str, unread_only=False):
        user = cls.__find_user_by_name(db_session, username)
    ...
    @classmethod
    def __find_user_by_name(cls, db_session, username: str):
        user = cls.get_user_by_name(db_session=db_session, username=username, create_if_not_exist=False)
